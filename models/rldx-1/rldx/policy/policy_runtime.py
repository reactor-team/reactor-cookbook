        ...
        if client_prefix is not None:
            ...
            for b in range(B_):
                ...
                for k, w in zip(modality_keys, joint_dims):
                    ...
                    cursor += w
                # The prefix came from actions the client is actually executing.
                # Do not apply the processor's training-time outlier clipping:
                # decoded model actions can legitimately normalize outside
                # [-1, 1], and clipping them here changes the scheduled actions
                # before they become the guided target or trained-mode prefix.
                norm_dict = sap.apply_action(
                    action_dict,
                    embodiment_tag=tag,
                    state=None,
                    clip_outliers=False,
