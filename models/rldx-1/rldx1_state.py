...
(same role ``rldx1_schema.py`` plays for the handshake).
...
frames, and :meth:`RLDXPipeline._resolve_state` prefers the committed tag.
"""
...
def parse_state(
    state_json: str, state_dims: Mapping[str, int]
...
class FrameStateTags:
    ...
    def offer(self, tag: bytes | None, capture_time_us: int | None = None) -> None:
        ...
        selection standing. The pipeline clears the selection between aligned
        commits so an older commit's tag cannot leak into a newer one.
        """
        if not tag:
            return
        self._arrivals += 1
