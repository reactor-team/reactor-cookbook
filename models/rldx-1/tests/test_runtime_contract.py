...
import rldx1 as pipeline  # noqa: E402
from rldx1_rtc import RTCRequestMailbox, RTCTiming  # noqa: E402
from rldx1_state import FrameStateTags  # noqa: E402
from rldx1_types import (  # noqa: E402
    ActionPrediction,
    CommandError,
    ModelSchema,
    RLDXInput,
...
def test_read_config_parses_the_repo_config():
    ...
    assert config["rtc_inference_mode"] == "guided"
    assert config["rtc_inference_delay"] == 5
    assert config["rtc_inference_exec_horizon"] == 8
...
def test_action_prediction_echoes_the_source_stamp_and_view_skew():
    chunk = ActionPrediction(
        source_capture_us=1_700_000_000_000_000, source_seq=41, view_skew_us=640
    )
...
def test_action_prediction_carries_rtc_install_metadata():
    chunk = ActionPrediction(
        request_id=7,
        plan_id=7,
        base_plan_id=6,
    ...
    assert chunk.plan_id == 7
    assert chunk.base_plan_id == 6
    assert chunk.install_step == 41
    assert chunk.rtc_prefix_len == 2


def test_request_action_event_queues_a_valid_cold_start():
    p = pipeline.RLDXPipeline()
    p._rtc_timing = RTCTiming(
        action_horizon=16,
        delay=2,
        exec_horizon=8,
        enabled=True,
    ...
    pending = p._rtc_requests.pending
    assert pending is not None
    assert pending.request_id == 0
    assert pending.base_plan_id == -1


def test_request_action_event_reports_a_bad_plan_chain():
    p = pipeline.RLDXPipeline()
    p._rtc_timing = RTCTiming(
        action_horizon=16,
        delay=2,
    ...
    asyncio.run(
        p.request_action(
            ...
            install_step=10,
            rtc_prefix_len=2,
            action_prefix=[[0.0] * 12, [0.0] * 12],
        )
    )

    assert p._rtc_requests.pending is None
    assert len(sent) == 1
    assert isinstance(sent[0], CommandError)
    assert "does not match active plan" in sent[0].reason
