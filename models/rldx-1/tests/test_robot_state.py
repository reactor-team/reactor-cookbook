...
``rldx1_state`` is pure (numpy only), so these run on a bare checkout without
...
from rldx1_state import FrameStateTags, parse_state, zero_state
...
def test_non_finite_stamp_is_ignored(key, stamp):
    tags = FrameStateTags()
