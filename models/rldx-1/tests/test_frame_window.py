...
"""Tests for capture-time alignment and its attached state metadata."""
...
from rldx1 import _align_by_capture, _AlignedCommit, _FrameCandidate
from rldx1_state import FrameStateTags
    ...
    commit = _align_by_capture(
        {
            ...
            "right_view": [
                candidate(1_100, Frame("right-old")),
                candidate(2_050, right),
            ],
            "wrist_view": [
                candidate(1_050, Frame("wrist-old")),
                candidate(1_980, wrist),
            ],
        }
    )
    assert isinstance(commit, _AlignedCommit)
    assert commit.frames == {
        "left_view": left,
        "right_view": right,
        "wrist_view": wrist,
    }
    assert commit.view_skew_us == 70
    ...
    commit = _align_by_capture(
        {
            ...
            "right_view": [
                candidate(2_010, right),
                candidate(3_020, Frame("right@3020")),
            ],
            "wrist_view": [
                candidate(1_000, Frame("wrist@1000")),
                candidate(2_005, wrist),
            ],
        }
    )
    assert commit.frames == {
...
def test_state_uses_another_aligned_view_when_reference_view_is_untagged():
    commit = _align_by_capture(
        {
            "left_view": [candidate(2_000, Frame("left@2"))],
            "right_view": [candidate(2_010, Frame("right@2"), state_tag(2))],
            "wrist_view": [candidate(2_005, Frame("wrist@2"), state_tag(2))],
        }
    )
    assert commit.state_candidates[0].metadata == state_tag(2)
